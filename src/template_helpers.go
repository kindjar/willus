...
func FloatAsDateTime(timestamp float64, format string) (formatted string) {
    ...
    switch format {
        case "short":       formatted = unixTime.Format(shortDateTimeFormat)
        case "long":        formatted = unixTime.Format(longDateTimeFormat)
        case "shortDate":   formatted = unixTime.Format(shortDateFormat)
        case "shortTime":   formatted = unixTime.Format(shortTimeFormat)
        default:            formatted = unixTime.Format(longDateTimeFormat)
        }
    return
}
